defmodule Matrix do
  ...
  #@struct %Matrix{line: int, row: int, contents: [[float]]}
  #NOTE:
  ...
  def get_line(%Matrix{contents: c}, index) do
    Enum.at(c, index-1)
  end
  def get_row(%Matrix{contents: c}, index) do
    for x <- c, do: Enum.at(x, index-1)
  end
  def get_factor(%Matrix{contents: c}, l_index, r_index) do
  ...
  def expand(%Matrix{line: l_m, contents: c} = matrix, inhom_vector) do
  ...
  # basic calculation
  def add(
    %Matrix{line: f_l_m, row: f_r_n, contents: f_c},
    %Matrix{line: s_l_m, row: s_r_n, contents: s_c},
    is_sub \\ false
  ) do
  ...
  def multiply(
    %Matrix{line: f_l_m, row: f_r_n} = f_m,
    %Matrix{line: s_l_m, row: s_r_n} = s_m
  ) do
    ...
    %Matrix{line: f_l_m, row: s_r_n, contents: new_c}
  end
  ...
  defp swap_lines(%Matrix{contents: c} = matrix, f_id, s_id) do
    %Matrix{ matrix |
                   contents: swap_factors(c, f_id, s_id)}
  end
  defp swap_rows(%Matrix{contents: c} = matrix, f_id, s_id) do
    swap_row = fn l -> swap_factors(l, f_id, s_id) end
    %Matrix{ matrix |
                   contents: Enum.map(c, swap_row)}
  end

  defp mul_scalar_line(%Matrix{contents: c} = matrix, id, scalar) do
    new_l = Enum.map(get_line(matrix, id), &(&1*scalar))
    %Matrix{ matrix |
                   contents: List.replace_at(c, id-1, new_l)}
  end
  defp mul_scalar_row(%Matrix{contents: c} = matrix, id, scalar) do
  ...
  defp add_mul_line(%Matrix{contents: c} = matrix, f_id, s_id, scalar) do
  ...
  defp add_mul_row(%Matrix{contents: c} = matrix, f_id, s_id, scalar) do
  ...
  def print_matrix(%Matrix{line: l_m} = matrix) do
  ...
  def print_matrix_row(%Matrix{line: l_m} = matrix) do
    IO.puts ""
    for i <- 1..l_m do
      IO.write i
      IO.write ":: "
